...
| `skripty/velin.mjs`, `skripty/velin/` | Velín: místní 3D přehled, kdo z agentů pracuje a na čem, úkoly, PR a noční směna. Tlačítka posílají úkoly na noc, zastavují noc, schvalují PR a posouvají úkoly. Three.js je přibalené (`skripty/velin/vendor/VENDOR.md`) |
...
4. Stav sleduj ve Velínu: `npm run velin` ve vlastním terminálu. Otevře se prohlížeč s odkazem, který nese klíč pro tlačítka. Odkaz nikomu neposílej. Bez klíče (nebo když Velín spustí agent) jde jen prohlížet. Jednoduchý přehled bez 3D je na http://127.0.0.1:4380/prehled. Otázky pro tebe mají štítek `pro-vlastnika`. Board na GitHub Projects srovná `npm run board`. Projekt „Tým agentů“ s polem „Stav týmu“ se zakládá jednou, podle `node skripty/board.mjs --help`.
